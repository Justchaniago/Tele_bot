...
/** Frozen M4 business quantity policy. M3 remains responsible for token extraction. */
export function validateBusinessQuantity(domain: DomainId, lexicalQuantity: string): QuantityValidation {
  ...
  const pattern = domain === "DAILY_SO" ? INTEGER : DECIMAL;
  if (!pattern.test(token)) return invalid("quantity must be a non-negative number in the domain format");
  ...
  if (domain !== "DAILY_SO" && fractionDigits > 2) return invalid("quantity exceeds two decimal places");
  if (domain === "DAILY_SO" && decimalSeparator) return invalid("Daily SO quantity must be an integer");
